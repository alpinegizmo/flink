 ...
 */
@PublicEvolving
public interface TemporalListState<T> extends ListState<T> {

	/**
	 * Updates the namespace used in all accesses and updates to the list. Must be called before
	 * using {@link #get()}, {@link #add(Object)}, {@link #update(List)}, etc.
	 *
	 * @param time The time to be used as the namespace for subsequent operations on the list.
	 */
